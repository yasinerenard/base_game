...
class Missile(PyObject):
    ...
    @classmethod
    def try_launch_missile(cls, target):
        cls.launch_counter += 1
        if cls.launch_counter >= cls.launch_interval_frames:
            cls.launch_counter = 0
            rocket = Missile(hero.pos, (39, 10), spr_rocket, target)
        return None

    def update(self):
        self.target = self.find_closest_enemy()
        if self.target:
            self.move_to(self.target.pos, self.speed)
            self.look_at(self.target.pos, offset=-180)
        super().update()
    ...
    def create_explosion(self):
        explosion = Explosion(pos=self.pos, size=(100, 100), sprite=spr_explosion[0], duration=30)
        PyObject.all.append(explosion)
...
def spawn_enemy_outside_screen(interval_frames, enemy):
    ...
    if spawn_enemy_outside_screen.counter >= interval_frames:
        ...
        new_enemy = Enemy(pos=(100, 100), size=(64, 64), sprites=spr_monster1, life=1, damage=10, speed=2, target=hero.pos)
        new_enemy.pos = pos
        PyObject.all.append(new_enemy)

spawn_enemy_outside_screen.counter = 0

tmx_file = "your_map.tmx"
...
while gamerunning:
    ...
    new_missile = Missile.try_launch_missile( enemy.pos)
    spawn_enemy_outside_screen(120,enemy)
