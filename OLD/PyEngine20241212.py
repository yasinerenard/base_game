...
def debuging():
    ...
    if hero.rect.collidepoint(mouse_pos): print("lol")
    if key_is_triggered(pygame.MOUSEMOTION): print("click")
...
class Missile(PyObject):
    ...
    def constant(self):
        for enemy in Enemy.all:
            if self.rect.colliderect(enemy.rect):  # Check for collision
                print("collision")
                # Create an explosion at the player's position
                explosion = Explosion(self.rect.center,60)
...
class Explosion(PyObject):
    def __init__(self, pos, duration):
        super().__init__(pos, (100, 100), sprites=spr_explosion[0:10], animfps=10)
...
rocket = Missile(hero.pos,(39,10),spr_rocket,enemy.pos)
explosion = Explosion((200,300),120)
