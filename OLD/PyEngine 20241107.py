...
class Missile(PyObject):
    all = []
    launch_interval_frames = 120  # Class-level attribute for launch interval (120 frames)
    ...
    def try_launch_missile(cls, pos, size, sprites, target):
        ...
        if cls.launch_counter >= cls.launch_interval_frames:
            ...
            return Missile(pos, size, sprites, target)
...
def spawn_enemy_outside_screen(interval_frames, enemy_sprites, enemy_size, target, speed, life=100, damage=10):
    """
    Spawns an enemy outside the screen at a specified interval.

    Parameters:
        interval_frames (int): Number of frames between each enemy spawn.
        enemy_sprites (list): List of sprites for the enemy animation.
        enemy_size (tuple): Size of the enemy (width, height).
        target (pygame.Vector2): The target that enemies should move towards.
        speed (int or float): Movement speed of the enemy.
        life (int): The health points of the enemy.
        damage (int): The damage the enemy deals.
